...
const recursivelySolveTheGameOfLife = function(board, row, column, numOfRows, numOfCols) {
    ...
    const calculateTotalNeighbors = function(row, column) {
        ...
        for (let neighbor of possibleNeighbors) {
            ...
            totalSum += board[row + neighborRow][column + neighborCol]
        }
        return totalSum
    }
    ...
    for(let neighbor of possibleNeighbors) {
        const neighborRow = neighbor[0]
        const neighborCol = neighbor[1]
        recursivelySolveTheGameOfLife(board, row + neighborRow, column + neighborCol, numOfRows, numOfCols)
    }
}
...
var gameOfLife = function(board) {
    ...
    recursivelySolveTheGameOfLife(board, 0, 0, numOfRows, numOfCols)
    return board
}
